...
export default function EditAccountPage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const updates: Record<string, unknown> = {
      ...
      billing_reset_date: formData.get("billing_reset_date"),
    };
  ...
  return (
    <div className="max-w-2xl">
      ...
      <form onSubmit={handleSubmit} className="card space-y-6">
        ...
        <div>
          <label htmlFor="billing_reset_date" className="label">Billing Reset Date</label>
          <input id="billing_reset_date" name="billing_reset_date" type="datetime-local" className="input" defaultValue={String(record.billing_reset_date ?? "")} />
        </div>
